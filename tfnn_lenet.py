...
weights = {
    ...
    # FC Layer 1: 7*7*64 inputs, 120 units.
    'wd1': tf.Variable(random_normal([7*7*16, fc1_units])),
    ...
    'wout': tf.Variable(random_normal([fc2_units, num_classes]))
}
...
def lenet(x):
    ...
    # Convolution Layer. Output shape: [-1, 14, 14, 16].
    conv2 = conv2d(conv1, weights['wc2'], biases['bc2'])
    
    # Max Pooling (down-sampling). Output shape: [-1, 7, 7, 16].
    conv2 = avgpool2d(conv2, k=2)

    # Reshape conv2 output to fit fully connected layer input, Output shape: [-1, 7*7*16].
    ...
    # Second Fully connected layer, Output shape: [-1, 84].
